...
SearchThread::SearchThread(ThreadPool &threadPool, uint32_t id)
    : numaId(Numa::DefaultNumaNodeId), running(false), exit(false), id(id),
      threads(threadPool) {}
...
void SearchThread::init(bool bindGroup) {
  ...
  runTask([bindGroup](SearchThread &th) {
    ...
    if (bindGroup) {
      ...
      // threshold, in this case all this NUMA machinery is not needed. We also
      // store this thread's numa ID for the later NUMA-aware loading of
      // evaluator weights.
...
void SearchThread::clear() {
  ...
  if (threads.dbStorage() &&
      (!dbClient || &dbClient->getStorage() != threads.dbStorage())) {
    dbClient = std::make_unique<Database::DBClient>(
        *threads.dbStorage(), Database::RECORD_MASK_LVDB,
        Config::DatabaseCacheSize, Config::DatabaseRecordCacheSize);
  }
}
...
void SearchThread::setBoardAndEvaluator(const Board &board) {
  ...
  if (!threads.evaluatorMaker)
    ...
    if (evaluator &&
        (evaluator->boardSize != boardSize || evaluator->rule != rule))
      evaluator.reset();
...
void MainSearchThread::checkExit(uint32_t elapsedCalls) {
  ...
  if (searchOptions.maxNodes &&
          threads.nodesSearched() >= searchOptions.maxNodes ||
...
void MainSearchThread::runCustomTaskAndWait(
    std::function<void(SearchThread &)> task, bool includeSelf) {
  if (!task)
    return;
...
void ThreadPool::startThinking(const Board &board, const SearchOptions &options,
                               bool inPonder, std::function<void()> onStop) {
  assert(size() > 0);
  assert(searcher());
  ...
  Opening::expandCandidate(*main()->board, main()->searchOptions.rule.rule);

  auto addMoveToRootMoves = [this](Pos m) {
    // Ignore blocked moves
    if (std::count(main()->searchOptions.blockMoves.begin(),
                   main()->searchOptions.blockMoves.end(), m))
      return;

    if (main()->searchOptions.balanceMode ==
        Search::SearchOptions::BALANCE_TWO) {
      ...
      MovePicker movePicker2(main()->searchOptions.rule, *main()->board,
  ...
  MovePicker movePicker(options.rule, *main()->board,
                        MovePicker::ExtraArgs<MovePicker::ROOT>{});
  ...
  if (options.balanceMode != Search::SearchOptions::BALANCE_TWO) {
    ...
    if (rootMoveList.size() < main()->rootMoves.size()) {
      ...
      main()->rootMoves.erase(std::remove_if(main()->rootMoves.begin(),
                                             main()->rootMoves.end(), pred),
                              main()->rootMoves.end());
    }
  }
  ...
  main()->runTask([this, searcher = searcher(),
                   onStop = std::move(onStop)](SearchThread &th) {
    searcher->searchMain(static_cast<MainSearchThread &>(th));
    if (onStop) // If onStop is set, queue a tail task to call it
      main()->runTask(
          [onStop = std::move(onStop)](SearchThread &th) { onStop(); });
  });
}
