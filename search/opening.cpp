...
std::vector<Pos> generateOpening(const Board &board, GameRule rule) {
  ...
  case GameRule::SWAP1:
    if (board.ply() == 0 && board.size() >= 13) {
      ...
      std::uniform_int_distribution<> choice(0,
                                             balanceCoordIncrements.size() - 1);
...
public:
  explicit IndexDisjointSet(size_t size) noexcept
      : root(size), size(size, 1), numSets(size) {
...
void expandCandidate(Board &board, Rule rule) {
  for (int i = 0; i < board.ply(); i++) {
    ...
    if (isNearBorder(board, pos, 0))
      board.expandCandArea(rule, pos, 4, 0);
    else if (isNearBorder(board, pos, 2))
      board.expandCandArea(rule, pos, 3, 0);
    else
      break;
  }
}
...
void expandCandidateHalfBoard(Board &board, Rule rule) {
  int c = board.size() / 4;
  board.expandCandArea(rule, Pos{c, c}, c + 1, 0);
}
...
void filterSymmetryMoves(const Board &board, std::vector<Pos> &moveList) {
  ...
  moveList.erase(std::remove_if(moveList.begin(), moveList.end(), pred),
                 moveList.end());
}
...
OpeningGenerator::OpeningGenerator(int boardSize, Rule rule,
                                   OpeningGenConfig cfg, PRNG prng)
    : board(boardSize), rule(rule), config(cfg), prng(prng) {
...
bool OpeningGenerator::next() {
  ...
  int idx =
      std::uniform_int_distribution<int>(0, numMoveChoices.size() - 1)(prng);
  ...
  if (numRandomMoves > 0) {
    ...
    int areaSize = std::uniform_int_distribution<>(config.localSizeMin,
                                                   config.localSizeMax)(prng);
    int x0 =
        std::uniform_int_distribution<>(0, board.size() - 1 - areaSize)(prng);
    int y0 =
        std::uniform_int_distribution<>(0, board.size() - 1 - areaSize)(prng);
  ...
  if (board.ply() == 0)
    board.expandCandArea(rule, board.centerPos(),
                         std::uniform_int_distribution<>(
                             board.size() / 4, board.size() / 2 + 1)(prng),
                         0);
...
void OpeningGenerator::putRandomMoves(int numMoves, CandArea area) {
  ...
  for (Pos pos : randomMoves) {
    ...
    if (board.p4Count(board.sideToMove(), A_FIVE) ||
        board.p4Count(board.sideToMove(), B_FLEX4) ||
...
bool OpeningGenerator::putBalance1Move() {
  ...
  if (options.maxNodes > 0) {
    ...
    if (std::abs(Threads.main()->rootMoves[0].value) >
        config.balance1FastCheckWindow)
      return false;
  }

  // Start the actual BALANCE1 search
  options.maxNodes =
      std::max(config.balance1Nodes, config.balance1FastCheckNodes) -
