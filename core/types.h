...
constexpr Color operator~(Color p) { return Color(p ^ 1); }
...
inline Value &operator+=(Value &d1, Value d2) { return d1 = d1 + d2; }
inline Value &operator-=(Value &d1, Value d2) { return d1 = d1 - d2; }
constexpr Value operator*(int i, Value d) { return Value(i * int(d)); }
constexpr Value operator*(Value d, int i) { return Value(int(d) * i); }
constexpr Value operator/(Value d, int i) { return Value(int(d) / i); }
constexpr int operator/(Value d1, Value d2) { return int(d1) / int(d2); }
inline Value &operator*=(Value &d, int i) { return d = Value(int(d) * i); }
inline Value &operator/=(Value &d, int i) { return d = Value(int(d) / i); }
...
constexpr Value storedValueToSearchValue(int storedValue, int searchPly) {
  return storedValue == VALUE_NONE              ? VALUE_NONE
         ...
         : storedValue <= VALUE_MATED_IN_MAX_PLY
             ? Value(storedValue + searchPly)
             : Value(storedValue);
}
...
enum Rule : uint8_t { FREESTYLE, STANDARD, RENJU, WRAP_1D, RULE_NB };
...
/// CandidateRange represents the options for different range of move
/// condidates.
