...
struct Pattern2x {
  ...
  Pattern operator[](Color side) const {
    return side == BLACK ? patBlack : patWhite;
  }
};
...
template <Rule R>
constexpr int HalfLineLen =
    (R == Rule::FREESTYLE || R == Rule::WRAP_1D) ? 4 : 5;

template <Rule R> constexpr int KeyLen = HalfLineLen<R> * 4;

template <Rule R> constexpr int KeyCnt = 1 << KeyLen<R>;
...
template <Rule R> inline uint64_t fuseKey(uint64_t key) {
  static_assert(HalfLineLen<R> == 4 || HalfLineLen<R> == 5,
                "incorrect half line length");
...
template <Rule R> inline Pattern2x lookupPattern(uint64_t key) {
  static_assert(R == FREESTYLE || R == STANDARD || R == RENJU || R == WRAP_1D,
                "incorrect rule to lookup pattern");

  key = fuseKey<R>(key);

  if constexpr (R == Rule::FREESTYLE || R == Rule::WRAP_1D)
...
inline uint8_t lookupDefenceTable(uint64_t key, Color attackSide) {
  ...
  //              | ((uint64_t)reverseByte(uint8_t(key >> 16)) >> 4);  //
  //              Reverse 20-bits
  //    else
  //        key = ((uint64_t)reverseByte(uint8_t(key)) << 8)
  //              | (uint64_t)reverseByte(uint8_t(key >> 8));  // Reverse
  //              16-bits
  //}
