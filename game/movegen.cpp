...
constexpr bool preCheckFilter(const Board &board, Color side) {
  if (bool(Type & VCF)) {
    if constexpr (bool(Type & COMB)) {
      if (board.p4Count(side, D_BLOCK4_PLUS) +
...
ScoredMove *findAllPseudoFourDefendPos(const Board &board, Color side,
                                       ScoredMove *moveList) {
  FOR_EVERY_CAND_POS(&board, pos) {
    const Cell &c = board.cell(pos);
    ...
    if (c.pattern4[side] >= E_BLOCK4)
      *moveList++ = pos;
      ...
      for (int dir = 0; dir < 4; dir++) {
        // Check if this pos is a B4 + (B4/F4), but recognized as FORBID in
        // Renju. If true, this is still a defend four pos for white.
...
ScoredMove *findFourDefence(const Board &board, ScoredMove *const moveList) {
  ...
  // Find all defend pos for F3 attack line pattern (_*OOO*_, X*OOO**X,
  // _*O*OO*_, _O*O*O*O_)
  auto findF3LineDefence = [=, &board](Pos f4Pos, int dir,
...
ScoredMove *findB4F3Defence(const Board &board, ScoredMove *const moveList) {
  ...
  auto findF3LineDefence = [=, &board](Pos f3Pos, int dir, ScoredMove *list) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if ((defenceMask >> (3 - i)) & 0x1) {
        ...
        *list++ = pos;
        ...
        if (checkRenjuDefence)
          foundLeftForbidden =
              foundLeftForbidden || board.checkForbiddenPoint(pos);
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if ((defenceMask >> (4 + i)) & 0x1) {
        ...
        *list++ = pos;
        ...
        if (checkRenjuDefence)
          foundRightForbidden =
              foundRightForbidden || board.checkForbiddenPoint(pos);
      } else if (checkRenjuDefence && prevFound && board.isEmpty(pos)) {
    ...
    if (checkRenjuDefence) {
      // If we have found a forbidden move for opponent black, we might have
      // some other possible defences. Normally the defence move should be at
      // the other side of the forbidden point, so we add defence move only in
      // this case.
  ...
  auto findB4InLine = [=, &board](Pos b4Pos, int dir) {
    ...
    auto checkNotOverlineB4 = [b4Pos, oppo, dir, &board](const Cell &c,
                                                         Pos pos) {
      Board &b = const_cast<Board &>(board);
      b.flipSide();
    ...
    for (i = 0; i < MaxFindDist; i++) {
      ...
      if (const Cell &c = board.cell(pos); c.piece == oppo)
        continue;
                                    ...
                                    R == RENJU && c.pattern4[oppo] == FORBID &&
                                        checkRenjuF4(c, pos))) {
  ...
  auto findAllB3CounterDefence = [=, &board](Pos b4Pos, int dir,
                                             ScoredMove *list) {
    Color self = ~oppo;
    const bool isPseudoForbiddenB4 = R == RENJU && self == BLACK &&
                                     board.cell(b4Pos).pattern4[self] == FORBID;
    ...
    for (int i = 0; i < MaxFindDist; i++) {
      ...
      if (const Cell &c = board.cell(pos); c.piece == self)
        continue;
               ...
               (isPseudoForbiddenB4 || c.pattern(self, dir) >= B3)) {
        *list++ = pos;
        continue;
      }
      break;
...
ScoredMove *generateFourDefence(const Board &board, ScoredMove *moveList) {
  ...
  std::sort(moveList, last,
            [](ScoredMove m, ScoredMove n) { return m.pos < n.pos; });
  last = std::unique(moveList, last,
                     [](ScoredMove m, ScoredMove n) { return m.pos == n.pos; });
...
template <GenType Type>
ScoredMove *generateNeighbors(const Board &board, ScoredMove *moveList,
                              Pos center, const Direction *neighbors,
                              size_t numNeighbors) {
  Color self = board.sideToMove();
  ...
  for (size_t i = 0; i < numNeighbors; i++) {
    ...
    if (c.piece == EMPTY && c.isCandidate() &&
        basicPatternFilter<Type>(board, pos, self))
      *moveList++ = pos;
  }
...
template ScoredMove *generateNeighbors<VCF>(const Board &, ScoredMove *, Pos,
                                            const Direction *, size_t);
template ScoredMove *generateNeighbors<VCF | COMB>(const Board &, ScoredMove *,
                                                   Pos, const Direction *,
                                                   size_t);

/// Generate direct winning moves for current side to move.
/// @return The first found winning pos.
/// @note Board state must satisfy `p4Count(self, A_FIVE) + p4Count(self,
/// B_FLEX4) > 0`.
template <>
...
ScoredMove *generate<DEFEND_FIVE>(const Board &board, ScoredMove *moveList) {
  ...
  *moveList = board.stateInfo().lastPattern4(oppo, A_FIVE);
  ...
  // In case of weird history, we find the A_FIVE pos by iterating all move
  // candidates.
  FOR_EVERY_CAND_POS(&board, pos) {
...
Pos nextPosWrap(Pos pos, int dir, int boardSize) {
  int idx = pos.y() * boardSize + pos.x();
  int strides[] = {1, boardSize, boardSize + 1, boardSize - 1};
  int nIdx = idx + strides[dir];
  if (nIdx < 0 || nIdx >= boardSize * boardSize)
    return Pos::NONE;
  return Pos(nIdx % boardSize, nIdx / boardSize);
}

Pos prevPosWrap(Pos pos, int dir, int boardSize) {
  int idx = pos.y() * boardSize + pos.x();
  int strides[] = {1, boardSize, boardSize + 1, boardSize - 1};
  int nIdx = idx - strides[dir];
  if (nIdx < 0 || nIdx >= boardSize * boardSize)
...
ScoredMove *generateB4F3DefenceWrap1D(const Board &board,
                                      ScoredMove *const moveList) {
  Color oppo = ~board.sideToMove();
  ...
  auto findF3LineDefence = [=, &board](Pos f3Pos, int dir, ScoredMove *list) {
    uint64_t key = board.getKeyAt<WRAP_1D>(f3Pos, dir);
    uint32_t defenceMask =
        PatternConfig::lookupDefenceTable<WRAP_1D>(key, oppo);

    Pos pos = f3Pos;
    for (int i = 0; i < 4; i++) {
      pos = prevPosWrap(pos, dir, boardSize);
      if (pos == Pos::NONE)
        break;
      if ((defenceMask >> (3 - i)) & 0x1) {
        assert(board.isEmpty(pos));
    ...
    pos = f3Pos;
    for (int i = 0; i < 4; i++) {
      pos = nextPosWrap(pos, dir, boardSize);
      if (pos == Pos::NONE)
        break;
      if ((defenceMask >> (4 + i)) & 0x1) {
        assert(board.isEmpty(pos));
  ...
  auto findAllB3CounterDefence = [=, &board](Pos b4Pos, int dir,
    ...
    for (int i = 0; i < MaxFindDist; i++) {
      ...
      if (const Cell &c = board.cell(pos); c.piece == self)
        continue;
      else if (c.piece == EMPTY && c.pattern(self, dir) >= B3) {
        *list++ = pos;
        continue;
      }
      break;
  ...
  if (const Cell &cell = board.cell(B4F3Pos);
    ...
    for (Pos p = Pos::FULL_BOARD_START; p < Pos::FULL_BOARD_END; p++) {
      if (p.isInBoard(boardSize, boardSize) && board.cell(p).piece == EMPTY &&
          board.cell(p).pattern4[oppo] == C_BLOCK4_FLEX3) {
        B4F3Pos = p;
        break;
      }
    }
...
template <>
ScoredMove *generate<DEFEND_FOUR | ALL>(const Board &board,
                                        ScoredMove *moveList) {
  return generateFourDefence<true>(board, moveList);
}

template <>
ScoredMove *generate<DEFEND_B4F3 | RULE_FREESTYLE>(const Board &board,
                                                   ScoredMove *moveList) {
  return generateB4F3Defence<FREESTYLE>(board, moveList);
}

template <>
ScoredMove *generate<DEFEND_B4F3 | RULE_STANDARD>(const Board &board,
                                                  ScoredMove *moveList) {
  return generateB4F3Defence<STANDARD>(board, moveList);
}

template <>
ScoredMove *generate<DEFEND_B4F3 | RULE_RENJU>(const Board &board,
                                               ScoredMove *moveList) {
  return generateB4F3Defence<RENJU>(board, moveList);
}

template <>
ScoredMove *generate<DEFEND_B4F3 | RULE_WRAP_1D>(const Board &board,
                                                 ScoredMove *moveList) {
  return generateB4F3DefenceWrap1D(board, moveList);
}
