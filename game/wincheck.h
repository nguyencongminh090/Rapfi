...
template <Rule Rule>
inline Value quickWinCheck(const Board &board, int ply,
  ...
  auto block4Count = [&](Color side) {
    return board.p4Count(side, C_BLOCK4_FLEX3) +
           board.p4Count(side, D_BLOCK4_PLUS) + board.p4Count(side, E_BLOCK4);
  };

  // FIXME: bug when multiple b4 are in the same line!
  // If opponent has more than a line of flex4, and we do not have any attack
  // moves with priority higher than B4, then we can not defend and will lose in
  // 3 steps.
  /*
  ...
  */
  ...
  // Check further mate types only in Freestyle and Standard, or we can improve
  // beta in Renju.
...
inline Value quickWinCheck(Rule rule, const Board &board, int ply,
                           Value beta = VALUE_MATE_IN_MAX_PLY) {
  switch (rule) {
  default:
  case FREESTYLE:
    return quickWinCheck<FREESTYLE>(board, ply, beta);
  case STANDARD:
    return quickWinCheck<STANDARD>(board, ply, beta);
  case RENJU:
    return quickWinCheck<RENJU>(board, ply, beta);
  case WRAP_1D:
    return quickWinCheck<WRAP_1D>(board, ply, beta);
  }
}
