...
/// Checks whether current p4Count in stateInfo matches that on board (used in
/// debug).
...
Board::Board(int boardSize, CandidateRange candRange)
    : boardSize(boardSize), boardCellCount(boardSize * boardSize), moveCount(0),
      passCount{0, 0}, currentSide(BLACK), currentZobristKey(0),
      candidateRange(nullptr), candidateRangeSize(0), evaluator_(nullptr),
      thisThread_(nullptr) {
...
Board::Board(const Board &other, Search::SearchThread *thread)
    : boardSize(other.boardSize), boardCellCount(other.boardCellCount),
...
template <Rule R> void Board::newGame() {
  ...
  FOR_EVERY_POSITION(this, pos) {
    ...
    valueBlack += c.valueBlack =
        Config::getValueBlack(R, pcode[BLACK], pcode[WHITE]);
  }
  st.valueBlack = valueBlack;
  st.candArea = CandArea();

  // For full board candidate range, we manually set all empty cells to
  // candidates.
  if (candidateRangeSize == 0)
    expandCandArea<FREESTYLE>(centerPos(), size() / 2, 0);
...
template <Rule R, Board::MoveType MT> void Board::move(Pos pos) {
  ...
  int f4CountBeforeMove[SIDE_NB] = {p4Count(BLACK, B_FLEX4),
                                    p4Count(WHITE, B_FLEX4)};
...
template <Rule R, Board::MoveType MT> void Board::undo() {
  assert(moveCount > 0);
...
// ----------------------------------------------------------------------------
// WRAP_1D Specializations
// ----------------------------------------------------------------------------

template <> inline uint64_t Board::getKeyAt<WRAP_1D>(Pos pos, int dir) const {
  constexpr int L = PatternConfig::HalfLineLen<WRAP_1D>;
  ...
  uint64_t key = 0;
  for (int i = -L; i <= L; ++i) {
    int nIdx = idx + i * stride;
    uint64_t cellVal = 0; // WALL
    if (nIdx >= 0 && nIdx < boardCellCount) {
      Pos nPos(nIdx % boardSize, nIdx / boardSize);
      Color piece = cells[nPos].piece;
      if (piece == EMPTY)
        cellVal = 3;
      else if (piece == BLACK)
        cellVal = 2; // 10b
...
template <> void Board::newGame<WRAP_1D>() {
  // Clear cells
  currentZobristKey = Hash::zobrist[BLACK][FULL_BOARD_CELL_COUNT - 1];
  for (Pos i = Pos::FULL_BOARD_START; i < Pos::FULL_BOARD_END; i++) {
    cells[i].piece = i.isInBoard(boardSize, boardSize) ? EMPTY : WALL;
  ...
  std::memset(static_cast<void *>(&stateInfos[0]), 0, sizeof(StateInfo));
  stateInfos[0].candArea = CandArea(0, 0, boardSize - 1, boardSize - 1);
  moveCount = 0;
  passCount[BLACK] = passCount[WHITE] = 0;
  currentSide = BLACK;
  currentZobristKey = 0;
  expandCandArea<WRAP_1D>(centerPos(), 0, 0);
  if (evaluator_)
    evaluator_->initEmptyBoard();
}

template <> void Board::move<WRAP_1D, Board::MoveType::NORMAL>(Pos pos) {
  if (UNLIKELY(pos == Pos::PASS)) {
    StateInfo &st = stateInfos[++moveCount];
    st = stateInfos[moveCount - 1];
    st.lastMove = Pos::PASS;
  ...
  constexpr int L = PatternConfig::HalfLineLen<WRAP_1D>;
  int idx = pos.y() * boardSize + pos.x();
  int strides[] = {1, boardSize, boardSize + 1, boardSize - 1};

  for (int i = -L; i <= L; i += 1 + (i == -1)) {
    for (int dir = 0; dir < 4; dir++) {
      int nIdx = idx + i * strides[dir];
      if (nIdx < 0 || nIdx >= boardCellCount)
        continue;

      Pos posi(nIdx % boardSize, nIdx / boardSize);
      Cell &c = cells[posi];
      if (c.piece != EMPTY)
        continue;
      ...
      c.pattern2x[dir] =
          PatternConfig::lookupPattern<WRAP_1D>(getKeyAt<WRAP_1D>(posi, dir));
      ...
      PatternCode pcode[SIDE_NB] = {c.pcode<BLACK>(), c.pcode<WHITE>()};
      deltaValueBlack += c.valueBlack =
          Config::getValueBlack(WRAP_1D, pcode[BLACK], pcode[WHITE]);

      st.p4Count[BLACK][c.pattern4[BLACK]]--;
      st.p4Count[WHITE][c.pattern4[WHITE]]--;
      c.updatePattern4AndScore<WRAP_1D>(pcode[BLACK], pcode[WHITE]);
      st.p4Count[BLACK][c.pattern4[BLACK]]++;
      st.p4Count[WHITE][c.pattern4[WHITE]]++;
  ...
  // Candidate update for WRAP_1D
  // Expand logical neighbors
  for (int stride : strides) {
    for (int k = 1; k <= candAreaExpandDist; ++k) {
      for (int sign : {-1, 1}) {
        int nIdx = idx + sign * k * stride;
        if (nIdx >= 0 && nIdx < boardCellCount) {
          Pos nPos(nIdx % boardSize, nIdx / boardSize);
          if (cells[nPos].piece == EMPTY && !cells[nPos].isCandidate())
            cells[nPos].cand++;
        }
      }
    }
  }
  // Also expand using candidateRange (visual neighbors) if needed?
  // The generic move uses candidateRange.
  // Let's stick to logical neighbors + visual neighbors if candidateRange is
  // set. But candidateRange is 2D.
  if (candidateRange) {
    for (size_t i = 0; i < candidateRangeSize; i++)
      cells[pos + candidateRange[i]].cand++;
  }
...
template <> void Board::move<WRAP_1D, Board::MoveType::NO_EVAL>(Pos pos) {
  // Similar to NORMAL but skip evaluator
  // For brevity, I'll implement it fully or call a helper?
  // Since I can't easily share code without refactoring, I'll duplicate the
  // logic but remove evaluator calls. It's safer to duplicate for now.
...
template <> void Board::undo<WRAP_1D, Board::MoveType::NORMAL>() {
  assert(moveCount > 0);
  Pos lastPos = getLastMove();
  ...
  constexpr int L = PatternConfig::HalfLineLen<WRAP_1D>;
  int idx = lastPos.y() * boardSize + lastPos.x();
  int strides[] = {1, boardSize, boardSize + 1, boardSize - 1};
...
template <> void Board::undo<WRAP_1D, Board::MoveType::NO_EVAL>() {
  assert(moveCount > 0);
  Pos lastPos = getLastMove();
...
template <> void Board::setBlock<WRAP_1D>(Pos pos) {
  assert(pos.valid());
  assert(pos.isInBoard(boardSize, boardSize));

  const Color oldPiece = cells[pos].piece;
  if (oldPiece == WALL)
    return;
  ...
  if (oldPiece == EMPTY) {
    deltaValueBlack -= cells[pos].valueBlack;
    st.p4Count[BLACK][cells[pos].pattern4[BLACK]]--;
    st.p4Count[WHITE][cells[pos].pattern4[WHITE]]--;
  } else if (oldPiece == BLACK || oldPiece == WHITE) {
    currentZobristKey ^= Hash::zobrist[oldPiece][pos];
  }

  cells[pos].piece = WALL;
...
template <Rule R> void Board::setBlock(Pos pos) {
  assert(pos.valid());
  assert(pos.isInBoard(boardSize, boardSize));
  ...
  StateInfo &st = stateInfos[moveCount]; // Get current state
  Value deltaValueBlack = VALUE_ZERO;

  if (oldPiece == EMPTY) {
    // This cell was empty, so it contributed to p4 counts and value.
    // We must remove its contribution.
    deltaValueBlack -= cells[pos].valueBlack;
    st.p4Count[BLACK][cells[pos].pattern4[BLACK]]--;
    st.p4Count[WHITE][cells[pos].pattern4[WHITE]]--;

    // An EMPTY cell has both BLACK and WHITE bits set (11b).
    // A WALL has them clear (00b). We flip both to clear them.
    flipBitKey(pos, BLACK);
    flipBitKey(pos, WHITE);
  } else if (oldPiece == BLACK || oldPiece == WHITE) {
    // This cell had a piece. Remove it from Zobrist hash.
    currentZobristKey ^= Hash::zobrist[oldPiece][pos];

    // A piece cell (e.g., BLACK) has the *opposite* bit set (WHITE, 10b).
    // We flip with the piece color (BLACK, 01b) to clear it.
    // E.g., 10b ^ 01b = 11b. Wait, this is wrong.
    // Let's re-check flipBitKey.
    // BLACK piece: flipBitKey(pos, BLACK) -> 11b ^ 01b = 10b (WHITE bit set)
    // WHITE piece: flipBitKey(pos, WHITE) -> 11b ^ 10b = 01b (BLACK bit set)
    // So, to clear BLACK piece (10b), we must flip WHITE: flipBitKey(pos,
    // WHITE) To clear WHITE piece (01b), we must flip BLACK: flipBitKey(pos,
    // BLACK)
    flipBitKey(pos, ~oldPiece);
  }

  // Set the piece to WALL. This cell is no longer empty.
  cells[pos].piece = WALL;

  // Expand candidate area around the new wall, just like a move.
  st.candArea.expand(pos, boardSize, candAreaExpandDist);
  for (size_t i = 0; i < candidateRangeSize; i++)
    cells[pos + candidateRange[i]].cand++;

  // Now, update all surrounding cells, identical to move()
  constexpr int L = PatternConfig::HalfLineLen<R>;
  int x = pos.x() + BOARD_BOUNDARY;
  int y = pos.y() + BOARD_BOUNDARY;
  uint64_t bitKey[4] = {
  ...
  for (int i = -L; i <= L; i += 1 + (i == -1)) {
    for (int dir = 0; dir < 4; dir++) {
      ...
      // Remove old pattern contribution
      deltaValueBlack -= c.valueBlack;
      st.p4Count[BLACK][c.pattern4[BLACK]]--;
      st.p4Count[WHITE][c.pattern4[WHITE]]--;

      // Recalculate line pattern
      c.pattern2x[dir] = PatternConfig::lookupPattern<R>(bitKey[dir]);

      // Get new combined patterns and value
      PatternCode pcode[SIDE_NB] = {c.pcode<BLACK>(), c.pcode<WHITE>()};
      deltaValueBlack += c.valueBlack =
          Config::getValueBlack(R, pcode[BLACK], pcode[WHITE]);

      // Update scores and p4 types
      c.updatePattern4AndScore<R>(pcode[BLACK], pcode[WHITE]);

      // Add new pattern contribution
      st.p4Count[BLACK][c.pattern4[BLACK]]++;
      st.p4Count[WHITE][c.pattern4[WHITE]]++;

      // Update last pattern4 move locations
      if (c.pattern4[BLACK] >= C_BLOCK4_FLEX3)
        st.lastPattern4Move[BLACK][c.pattern4[BLACK] - C_BLOCK4_FLEX3] = posi;
      if (c.pattern4[WHITE] >= C_BLOCK4_FLEX3)
        st.lastPattern4Move[WHITE][c.pattern4[WHITE] - C_BLOCK4_FLEX3] = posi;
  ...
  // Apply the accumulated value changes
  st.valueBlack += deltaValueBlack;

  assert(checkP4(this));

  // This is a permanent, non-undoable change. The safest way to
  // update the evaluator (NNUE) is to resynchronize it with the
  // board's current state.
  if (evaluator_)
    evaluator_->syncWithBoard(*this);
}

template void Board::setBlock<FREESTYLE>(Pos pos);
template void Board::setBlock<STANDARD>(Pos pos);
template void Board::setBlock<RENJU>(Pos pos);
...
bool Board::checkForbiddenPoint(Pos pos) const {
  ...
  for (int dir = 0; dir < 4; dir++) {
    ...
    if (fpCell.pattern2x[dir].patBlack == OL)
      return true;
    // Otherwise if it has at least two Four(B4/F4), it must be a true forbidden
    // point.
    else if (fpCell.pattern2x[dir].patBlack == B4 ||
...
void Board::expandCandArea(Pos pos, int fillDist, int lineDist) {
  ...
  auto candCondition = [&](Pos p) {
    return p >= 0 && p < FULL_BOARD_CELL_COUNT && isEmpty(p) &&
           !cell(p).isCandidate();
  };

  area.expand(pos, boardSize, std::max(fillDist, lineDist));

  if constexpr (R == WRAP_1D) {
    // 1D Logic: Expand logical neighbors
    int idx = y * boardSize + x;
    int strides[] = {1, boardSize, boardSize + 1, boardSize - 1};

    for (int stride : strides) {
      for (int k = 1; k <= lineDist; ++k) {
        for (int sign : {-1, 1}) {
          int nIdx = idx + sign * k * stride;
          if (nIdx >= 0 && nIdx < boardCellCount) {
            Pos nPos(nIdx % boardSize, nIdx / boardSize);
            if (candCondition(nPos))
              cells[nPos].cand++;
          }
        }
      }
    ...
    // Also include 2D fill for "Square" expansion (visual neighbors)
...
void Board::expandCandArea(Rule rule, Pos pos, int fillDist, int lineDist) {
  switch (rule) {
  case FREESTYLE:
    expandCandArea<FREESTYLE>(pos, fillDist, lineDist);
    break;
  case STANDARD:
    expandCandArea<STANDARD>(pos, fillDist, lineDist);
    break;
  case RENJU:
    expandCandArea<RENJU>(pos, fillDist, lineDist);
    break;
  case WRAP_1D:
    expandCandArea<WRAP_1D>(pos, fillDist, lineDist);
    break;
  }
}
