...
#define FOR_EVERY_CANDAREA_POS(board, pos, candArea)                           \
  for (int8_t _y = (candArea).y0, y1 = (candArea).y1, x0 = (candArea).x0,      \
              x1 = (candArea).x1, _x = x0;                                     \
       _y <= y1; _y++, _x = x0)                                                \
...
struct CandArea {
  ...
  CandArea(int8_t x0, int8_t y0, int8_t x1, int8_t y1)
...
/// Cell struct contains all information for a move cell on board, including
/// current stone piece, candidate, pattern, pattern4 and move score.
struct Cell {
  Color piece;
  uint8_t cand;
  ...
  /// Check if this cell is a move candidate, which can be used in move
  /// generation.
  ...
  template <Color C> PatternCode pcode() const {
...
public:
  ...
  explicit Board(int boardSize,
                 CandidateRange candRange = Config::DefaultCandidateRange);
  ...
  ///     4. NO_EVAL_MULTI: Updates cell, pattern, score. Side to move is not
  ///     swapped.
  ...
  template <Rule R, MoveType MT = MoveType::NORMAL> void move(Pos pos);

  /// Undo move and rollback the board state.
  /// @tparam R Game rule to use.
  /// @tparam MT Type of this move. Must match the move type used in move().
  template <Rule R, MoveType MT = MoveType::NORMAL> void undo();

  /// Set a permanent block (wall) on the board.
  /// @param pos Pos to set the block.
  /// @tparam R Game rule to use.
  template <Rule R> void setBlock(Pos pos);

  /// Overload for setBlock(Pos pos).
  template <Rule R> inline void setBlock(int x, int y) {
    setBlock<R>(Pos(x, y));
  }
  ...
  /// A dynamic dispatch version of setBlock().
  void setBlock(Rule rule, Pos pos);

  /// Overload for setBlock(Rule rule, Pos pos).
  inline void setBlock(Rule rule, int x, int y) { setBlock(rule, Pos(x, y)); }
  ...
  bool isLegal(Pos pos) const {
    return pos.valid() && (isEmpty(pos) || pos == Pos::PASS);
  }
  ...
  template <Rule R> uint64_t getKeyAt(Pos pos, int dir) const;
  ...
  uint16_t p4Count(Color side, Pattern4 p4) const {
    return stateInfo().p4Count[side][p4];
  }
  ...
  /// @return Pos::NONE if there is no such move, otherwise returns the actual
  /// move.
  ...
  template <Rule R> void expandCandArea(Pos pos, int fillDist, int lineDist);

  /// Expand candidate area with runtime rule.
  void expandCandArea(Rule rule, Pos pos, int fillDist, int lineDist);
...
template <Rule R> inline uint64_t Board::getKeyAt(Pos pos, int dir) const {
  assert(dir >= 0 && dir < 4);
  ...
  case 1:
    return rotr(bitKey1[x], 2 * (y - L));
  case 2:
    return rotr(bitKey2[x + y], 2 * (x - L));
  case 3:
    return rotr(bitKey3[FULL_BOARD_SIZE - 1 - x + y], 2 * (x - L));
  }
}
...
inline void Board::newGame(Rule rule) {
  assert(rule < RULE_NB);
  void (Board::*F[])() = {&Board::newGame<FREESTYLE>, &Board::newGame<STANDARD>,
                          &Board::newGame<RENJU>, &Board::newGame<WRAP_1D>};
  (this->*F[rule])();
}

inline void Board::move(Rule rule, Pos pos) {
  assert(rule < RULE_NB);
  void (Board::*F[])(Pos) = {&Board::move<FREESTYLE>, &Board::move<STANDARD>,
                             &Board::move<RENJU>, &Board::move<WRAP_1D>};
  (this->*F[rule])(pos);
}

inline void Board::undo(Rule rule) {
  assert(rule < RULE_NB);
  void (Board::*F[])() = {&Board::undo<FREESTYLE>, &Board::undo<STANDARD>,
                          &Board::undo<RENJU>, &Board::undo<WRAP_1D>};
  (this->*F[rule])();
}

inline void Board::setBlock(Rule rule, Pos pos) {
  assert(rule < RULE_NB);
  void (Board::*F[])(Pos) = {
      &Board::setBlock<FREESTYLE>, &Board::setBlock<STANDARD>,
      &Board::setBlock<RENJU>, &Board::setBlock<WRAP_1D>};
  (this->*F[rule])(pos);
}
