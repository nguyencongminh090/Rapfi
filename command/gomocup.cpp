...
void think(Board &board, uint16_t multiPV = 1,
           Search::SearchOptions::BalanceMode balanceMode =
               Search::SearchOptions::BALANCE_NONE,
           bool swapable = false, bool disableOpeningQuery = false) {
  ...
  thinking = true;
  Search::Threads.startThinking(
      board, options, false, [&, startTime = now()]() {
        {
#ifdef MULTI_THREADING
...
void getOption() {
  ...
  if (token == "TIMEOUT_TURN") {
    ...
    if (GUIMode && options.matchTime == 100000000 &&
        options.turnTime == 2000000)
    ...
    if (val == 2147483647) // unlimited match time specificed in the protocol
      ...
      options.timeLeft = val;
             ...
             token == "HASH_SIZE" /* Yixin-Board Extension*/) {
    std::cin >> val;
    ...
    size_t memLimitKB =
        maxMemSizeKB <= memReservedKB ? 1 : maxMemSizeKB - memReservedKB;
    ...
    case 0:
      ...
      break;
    case 1:
      options.rule = {Rule::STANDARD, GameRule::FREEOPEN};
      break;
    case 3:
      options.rule = {Rule::WRAP_1D, GameRule::FREEOPEN};
      break;
    ...
    case 4:
      ...
      break;
    case 5:
      options.rule = {Rule::FREESTYLE, GameRule::SWAP1};
      break;
    case 6:
      options.rule = {Rule::FREESTYLE, GameRule::SWAP2};
      break;
    default:
    ...
    if (Config::MemoryReservedMB[prevRule] !=
        Config::MemoryReservedMB[options.rule.rule]) {
  ...
  /////////////////////////////////////////////////
    ...
    switch (val) {
    default:
      MESSAGEL("Unknown show_detail, setting to 0.");
      [[fallthrough]];
    case 0:
      options.infoMode = Search::SearchOptions::INFO_NONE;
      break;
    case 1:
      options.infoMode = Search::SearchOptions::INFO_REALTIME;
      break;
    case 2:
      options.infoMode = Search::SearchOptions::INFO_DETAIL;
      break;
    case 3:
      options.infoMode = Search::SearchOptions::INFO_REALTIME_AND_DETAIL;
      break;
    }
    ...
    switch (val) {
    case 0:
      candRange = CandidateRange::SQUARE2;
      break;
    default:
      MESSAGEL("Unknown caution_factor, setting to 1.");
      [[fallthrough]];
    case 1:
      candRange = CandidateRange::SQUARE2_LINE3;
      break;
    case 2:
      candRange = CandidateRange::SQUARE3;
      break;
    case 3:
      candRange = CandidateRange::SQUARE3_LINE4;
      break;
    case 4:
      candRange = CandidateRange::SQUARE4;
      break;
    case 5:
      candRange = CandidateRange::FULL_BOARD;
      break;
    }
  ...
  /////////////////////////////////////////////////
    ...
    switch (val) {
    default:
      MESSAGEL("Unknown draw_result, setting to 0.");
      [[fallthrough]];
    case 0:
      options.drawResult = Search::SearchOptions::RES_DRAW;
      break;
    case 1:
      options.drawResult = Search::SearchOptions::RES_BLACK_WIN;
      break;
    case 2:
      options.drawResult = Search::SearchOptions::RES_WHITE_WIN;
      break;
    }
    ...
    Config::EvaluatorDrawRatio =
        std::clamp(Config::EvaluatorDrawRatio, 0.0f, 1.0f);
...
void showHashUsage() {
  ...
  MESSAGEL("Transposition table full: " << hashfull_permill / 10 << "."
                                        << hashfull_permill % 10 << "%");
}
...
void loadHash() {
  ...
  if (hashin.is_open() && Search::TT.load(hashin))
    MESSAGEL("Transposition table loaded successfully from "
             << pathToConsoleString(path));
  else
  ...
  if (Search::Threads.dbStorage()) {
    ...
    if (currentBoardSizeAndRule)
      filter = [&](const DBKey &key, const DBRecord &record) -> bool {
        return key.boardHeight == board->size() &&
               key.boardWidth == board->size() && key.rule == options.rule.rule;
      };
    ::Database::databaseToCSVFile(*Search::Threads.dbStorage(), txtout, filter);
    MESSAGEL("Wrote " << (currentBoardSizeAndRule
                              ? "(current boardsize and rule)"
                              : "(all)")
                      << " database to csv-format text file "
                      << pathToConsoleString(txtPath));
  }
}
...
void libToDatabase() {
  ...
  if (Search::Threads.dbStorage()) {
    ...
    size_t writeCount = ::Database::importLibToDatabase(
        *Search::Threads.dbStorage(), libStream, options.rule,
        board ? board->size() : 15);
    auto endTime = now();
...
void databaseToLib() {
  ...
  if (Search::Threads.dbStorage()) {
    ...
    if (!libStream) {
      ERRORL("Unable to open file " << pathToConsoleString(libPath)
                                    << " for writing.");
      return;
    }

    DBClient dbClient(*Search::Threads.dbStorage(), RECORD_MASK_ALL);
    size_t nodeCount = ::Database::exportDatabaseToLib(dbClient, libStream,
                                                       *board, options.rule);
    auto endTime = now();
...
void getBlock(bool remove = false) {
  ...
  while (true) {
    ...
    bool alreadyInList =
        std::count(options.blockMoves.begin(), options.blockMoves.end(), pos);
    if (!remove) {
      if (!alreadyInList)
        options.blockMoves.push_back(pos);
    } else if (alreadyInList) {
      options.blockMoves.erase(std::remove(options.blockMoves.begin(),
                                           options.blockMoves.end(), pos),
                               options.blockMoves.end());
    }
  }
}

void setBlockProtocol() {
  int x, y;
  char comma;
  std::string coordStr;
  std::stringstream ss;
  ...
  while (true) {
    ...
    Pos pos = inputCoordConvert(x, y, board->size());
    if (!board->isInBoard(pos) || pos == Pos::PASS) {
      ERRORL("setBlock coord is a pass or invalid.");
      continue;
    }

    // Call the new board function
    board->setBlock(options.rule, pos);
  }
  MESSAGEL("Board block(s) set.");
}
    ...
    FOR_EVERY_EMPTY_POS(board, pos) {
      if (board->checkForbiddenPoint(pos))
        std::cout << std::setfill('0') << std::setw(2)
                  << outputCoordXConvert(pos, board->size())
...
void balance(Search::SearchOptions::BalanceMode mode) {
  ...
  if (board->ply() == 0)
    Opening::expandCandidateHalfBoard(*board, options.rule.rule);
...
void queryDatabaseAll(bool getPosition) {
  ...
  if (Search::Threads.dbStorage()) {
    ...
    FOR_EVERY_EMPTY_POS(board, pos) {
      ...
      if (boardPosAndTextsIt != boardPosAndTexts.end() &&
          boardPosAndTextsIt->first == pos) {
    ...
    if (!Config::DatabaseReadonlyMode) {
      ...
      if (board->ply() > 0 && !dbClient.query(*board, options.rule, record))
        dbClient.save(*board, options.rule, DBRecord{LABEL_NONE},
                      OverwriteRule::Disabled);
    }
  }
}
  ...
  if (Search::Threads.dbStorage()) {
    ...
    if (dbClient.query(*board, options.rule, record) && !record.isNull())
      MESSAGEL("DATABASE TEXT "
               << std::quoted(UTF8ToConsoleCP(record.comment())));
    else
      MESSAGEL("DATABASE TEXT \"\"");
  }
}
...
void deleteDatabaseAll(bool getPosition) {
  ...
  if (auto firstChar = std::toupper(std::cin.peek());
    ...
    if (deleteType == "NONWL")
      ...
      deleteFilter = [](DBRecord &record) {
        return isDeterminedLabel(record.label)
                   ? DBClient::DelType::NoDelete
                   : DBClient::DelType::DeleteRecursive;
      };
      ...
      deleteFilter = [](DBRecord &record) {
        return record.label == LABEL_LOSE
                   ? DBClient::DelType::DeleteRecursive
                   : DBClient::DelType::NoDeleteRecursive;
      };
  ...
  if (Search::Threads.dbStorage() && !Config::DatabaseReadonlyMode) {
    ...
    auto endTime = now();
    MESSAGEL("Done deleting " << (sizeBeforeDelete - sizeAfterDelete)
                              << " records using " << (endTime - startTime)
                              << " ms.");
...
void splitDatabase() {
  ...
  if (Search::Threads.dbStorage()) {
    ...
    if (auto dbToSplit = Config::createDefaultDBStorage(dbPathUTF8)) {
      auto startTime = now();
      size_t writeCount = ::Database::splitDatabase(
          *Search::Threads.dbStorage(), *dbToSplit, *board, options.rule);
      auto endTime = now();
      MESSAGEL("Write " << writeCount
                        << " records into the spilted database using "
                        << (endTime - startTime) << " ms.");
    }
  }
}
...
void mergeDatabase() {
  ...
  if (Search::Threads.dbStorage()) {
    ...
    if (auto dbToMerge = Config::createDefaultDBStorage(dbPathUTF8)) {
      size_t writeCount =
          mergeDatabase(*Search::Threads.dbStorage(), *dbToMerge,
...
void traceSearch() {
  ...
  MESSAGEL("Root Moves(exclude symmetry)[" << moveList.size()
                                           << "]: " << MovesText{moveList});
  ...
  MESSAGEL("Static Eval[Black]: "
           << evalPOB << " (WDL " << std::fixed << std::setprecision(2)
  ...
  if (board.evaluator()) {
    ...
    MESSAGEL("Evaluator Eval[Black]: "
             << value << " WR: " << std::setprecision(4) << win
             << " LR: " << loss << " DR: " << v.draw());
  }
  ...
  Pos ttMove;
  int ttDepth;
  if (Search::TT.probe(board.zobristKey(), ttValue, ttEval, ttIsPv, ttBound,
                       ttMove, ttDepth, 0)) {
...
bool runProtocol() {
  ...
  // We do not stop thinking here, but let the outer loop to wait for its
  // finish.
  if (std::cin.eof())
    return true;
  ...
  // clang-format off
    ...
    else if (cmd == "YXSETBLOCK")          CheckBoardOK(setBlockProtocol);
